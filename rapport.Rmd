...
Nous avons trouvé ces données au niveau national et au niveau régional.
...
Pour conclure, nous avons pu en apprendre plus sur l'agglomération troyenne, que ce soit sur la taille des bâtiments, leur usage, leur nature, leur disposition dans la ville, etc.
Nous avons trouvé cela très intéressant car ce sont des informations sur la vile où nous vivons.
Nous nous sommes également renseignés sur différents domaines suite à des interrogations en voyant certaines données, que ce soit pour les confirmer, trouver une explication ou se rendre compte que la tendance n'est pas correcte car il nous manque des données.

Justement, des données il nous en manquait beaucoup sur certaines variables, notamment la nature des bâtiments qui était assez peu renseignée (moins de 10% des bâtiments), ce qui était assez limitant pour effectuer des analyses concluantes.
Pour la plupart des autres variables, nous avions 50% ou plus du jeu de données renseigné ce qui est plus représentatif du territoire.

Aussi, il n'était pas facile de lier les données que nous avons récupérées de différentes sources.
Nous n'avons pas réussi à lier la base de données des bâtiments avec celle des adresses.
Cette dernière n'a au final été utilisée que dans Tableau.

Nous avons néanmoins réussi à obtenir des coordonnées GPS à partir des données initiales.
Pour cela, nous avons utilisé le fichier `.gpkg` qui est une base de données géospatiale au format SQLite.
Effectivement, il n'y avait pas d'information sur l'emplacement dans le fichier `.csv`.
Nous avons eu du mal à récupérer la position des bâtiments car c'était un format de fichier que nous ne connaissions pas.

Nous avons également rencontrés des difficultés pour les visualisations spatiales de par leur nature plus complexe à mettre en oeuvre, surtout en R.
Aussi, nous avons eu des problèmes avec l'import des dates dans Tableau.
En effet, lors de leur conversion il n'est pas possible d'avoir des dates plus anciennes que 1900.

Il pourrait y avoir plusieurs continuations à ce travail exploratoire du jeu de données des bâtiments de la métropole troyenne.
Tout simplement, une continuation de l'exploration, plus de croisements avec d'autres bases de données qui pourraient compléter les données manquantes ou ajouter des variables.
Nous pourrions également imaginer, à la suite du croisement des données avec les adresses, de pouvoir entrer une adresse et avoir des informations sur le bâtiment correspondant.
...
À travers ce projet, j'ai pu apprendre à utiliser des outils afin d'explorer un jeu de données.
J'ai notamment compris l'importance de visualiser des données afin de pouvoir s'approprier le jeu de données.
J'ai aussi appris sur le plan technique à réaliser des visualisations avec R, que ce soit avec ggplot ou avec plotly avec Shiny.
Il est beaucoup plus intéressant à mon sens d'avoir un graphique interactif qu'un graphique statique.

Ayant principalement travaillé sur des représentations spatiales, j'ai rencontrés plusieurs limitations avec R, notamment la difficulté d'ajouter un fond de carte à un graphique.
Mais surtout j'ai réalisé un export des coordonnées GPS des bâtiments à l'aide du fichier `.gpkg`, hors j'ai uniquement exporté un point et non la forme du bâtiment.
Cela aurait été beaucoup plus compliqué mais cela aurait pu être pertinent pour réaliser certaines représentations.
...
Une grande partie des données que j'ai utilisé ne sont pas définies, ce qui est un peu frustrant car il est difficile de donner une interprétation satisfaisante des visualisations qui en découlent.

#### Danielle :

Durant ce projet, j'ai appris à manipuler les données d'un dataset. Comment nettoyer des données et en faire sortir des informations.
Par rapport aux difficultés rencontrées, j'ai eu un peu de mal au début à m'habituer à R mais par la suite,je me suis un peu améliorée. Ensuite, pour les visualisations, il y a avait des données pas assez explicites (ex: nature ou usage indifférencié) et c'était un frein aux interprétations.
