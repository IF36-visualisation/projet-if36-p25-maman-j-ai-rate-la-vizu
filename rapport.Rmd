...
ggplot(batiments6, aes(x = ANNEE, fill = USAGE1, color = USAGE1)) +
  geom_density(aes(y = after_stat(density)), alpha = 0.4, adjust = 1) +
  scale_fill_manual(values = palette_uses) +
  scale_color_manual(values = palette_uses) +
  scale_y_continuous(trans = power_trans(power = 1/4)) +
  labs(
    title = "Distribution racine carrée de l'année de construction selon l'usage",
    x = "Année de construction",
    y = "Densité (sqrt)",
    fill = "Usage principal",
    color = "Usage principal"
  )
