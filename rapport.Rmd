...
*L’objectif est de déterminer si l’année de construction d’un bâtiment a une influence sur son usage principal (`usage1`). L’hypothèse sous-jacente est que certains types d’usage sont plus fréquents pour des bâtiments récents (par exemple les bâtiments tertiaires ou commerciaux), tandis que d'autres usages (comme l'habitat individuel ou collectif) sont plus répandus dans des bâtiments anciens.*

*Nous utiliserons la variable `annee_construction` croisée avec `usage1`, et éventuellement `usage2`, afin de visualiser les évolutions des usages au fil du temps.*

**Pour cela, nous envisageons :**

-   *Un graphique en barres ou en aires empilées représentant la répartition des usage1 par tranche d’années de construction (ex. : avant 1950, 1950–1970, etc.).*
-   *Un diagramme de densité ou histogramme montrant la distribution de l’année de construction pour chaque type d’usage principal.*
...
-   *Une analyse croisée avec nb_lgmts pour voir si l’évolution dans le temps des usages s’accompagne d’une évolution dans la densité résidentielle.*

**Les problèmes potentiels :**

-   *La variable annee_construction peut être manquante ou imprécise pour certains bâtiments (valeurs nulles, erronées ou hors du timestamp).*
-   *Il faudra regrouper les années par tranches cohérentes pour faciliter l’analyse (ex. : décennies ou périodes historiques de développement urbain).*
...
-   *Des biais peuvent exister dans la base si certains types d'usages sont surreprésentés dans certaines périodes.*
...
### Peut-on voir des matériaux de construction des murs de prédilection selon les époques ?
...
### Peut-on voir des matériaux de construction des toits de prédilection selon les époques ?
...
Sans surprise, les tuiles sont le matériau le plus utilisé pour les toits. Au niveau national, les tuiles représentent 74% des toitures françaises.
Les autres matériaux fortement utilisés sont le zinc aluminium et l'ardoise. 
Si nous n'avons pas trouvé de chiffres précis pour ces matériaux, ils font partis des matériaux les plus utilisés pour les toitures en France. Il n'est donc pas surprenant de les retrouver dans le graphe.

### Quel est la part de maison (1 seul logement dans le bâtiment) et d'appartement parmis les résidences?

Variables utilisées: NB_LOGTS (nombre de logements par batiment), USAGE1 et USAGE2
Moyenne nationale selon FNAIM: 55% de maison, 45% d'appartement
Moyenne dans l'Aube selon FNAIM: entre 20% et 40% d'appartement
...
ggplot(batiment_pourcentage, aes(x = NB_LOGTS, y = pct)) +
  ...
  On observe que seulement 1% des batiments sont des logements uniques. Cette information est surprenante, d'autant que le pourcentage de bâtiment à logement unique à Paris est de 0,88%.
  Il parraît improbable qu'il y ait si peu de maison à Troyes. Nous remarquons que la quasi totalité des bâtiments "indifférencié" ont un nombre inconnu de logement.
  ...
  Cela n'est probablement pas dû aux colocations. En effet, un logement doit être composé au minimum d'une chambre, une cuisine et une salle de bain (la chambre et la cuisine peuvent être dans la même pièce).
  ...
  ### Quel est la part de batiments résidentiels à 2 logements sont aussi commerciaux?
